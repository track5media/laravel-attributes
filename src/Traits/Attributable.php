...
trait Attributable
{
    ...
    public function getEntityAttributes(): Collection
    {
        ...
        if (! static::$entityAttributes->has($morphClass) && Schema::hasTable(config('rinvex.attributes.tables.attribute_entity'))) {
            ...
            static::$entityAttributes->put($morphClass, app('rinvex.attributes.attribute')->whereIn('id', $attributes)->orderBy('sort_order', 'ASC')->orderByRaw('json_extract(name, ?) asc', ['$.' . $locale])->get()->keyBy('slug'));
        }
